...
public class GenresFuserIntersection extends
		AttributeValueFuser<List<String>, Books, Attribute> {

	public GenresFuserIntersection() {
		super(new Intersection<String, Books, Attribute>());
	}

	@Override
	public boolean hasValue(Books record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Books.GENRES);
	}

	@Override
	public List<String> getValue(Books record, Correspondence<Attribute, Matchable> correspondence) {
	...
	public void fuse(RecordGroup<Books, Attribute> group, Books fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
			...
			FusedValue<List<String>, Books, Attribute> fused = getFusedValue(group, schemaCorrespondences, schemaElement);
			fusedRecord.setGenres(fused.getValue());
			fusedRecord.setAttributeProvenance(Books.GENRES, fused.getOriginalIds());
