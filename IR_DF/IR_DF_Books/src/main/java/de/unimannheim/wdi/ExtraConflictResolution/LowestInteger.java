 ...
 */
public class LowestInteger<RecordType extends Matchable & Fusible<SchemaElementType>, SchemaElementType extends Matchable> extends
	...
	public FusedValue<String, RecordType, SchemaElementType> resolveConflict(
		...
		FusibleValue<String, RecordType, SchemaElementType> lowest = null;

		for (FusibleValue<String, RecordType, SchemaElementType> value : values) {
			
			String extractedPrice = value.getValue();
			int convertedPrice;
			
			try {
				convertedPrice = Integer.parseInt(extractedPrice);
			} catch (Exception e) {
				System.out.println("Could not convert priceString to Int: " + extractedPrice);
				continue;
			}
			
			if (lowest == null || convertedPrice < Integer.parseInt(lowest.getValue())) {
				lowest = value;
			}
		}

		return new FusedValue<>(lowest);
	}
